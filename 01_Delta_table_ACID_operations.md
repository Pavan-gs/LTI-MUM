...
# Read table
df = spark.sql("SELECT * FROM lakehouse.dbo.people LIMIT 1000")
display(df)
...
# Read table
df = spark.sql("SELECT * FROM master_lakehouse.dbo.people LIMIT 1000")
display(df)
```
...
```sql
%%sql
DROP TABLE IF EXISTS master_lakehouse.dbo.people;
```
## **9. Conclusion**
...
1. **Explore Schema Evolution**: Delta supports automatic schema evolution when new columns are added to the data.Here is [handson]
